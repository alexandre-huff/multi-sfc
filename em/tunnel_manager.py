...
class GRETunnel:
    ...
    def __init__(self, ip_address, port=8000):
        ...
        logger.info("Polling GRE EM endpoint at %s:%s", ip_address, port)

        try:
            socket_polling(ip_address, port)

        except (OSError, TimeoutError) as err:
            raise TimeoutError("GRE management interface at %s:%s is closed. %s." % (ip_address, port, err))

    def get_networks(self):
        networks = requests.get(self.base_url + '/networks/gre', headers=headers, timeout=15)

        return networks.json()

    def start(self, local_ip, remote_ip, server, route_net_host, nat):
        """Starts one endpoint of a GRE Tunnel

        :param local_ip: wan ip
        :param remote_ip:
        :param server: True if server, False otherwise
        :param route_net_host: the route to be installed to route gre traffic to the remote host
        :param nat: True if use MASQUERADE, False otherwise
        :return:
        ...
        if server:
            data['gre_local_ip'] = '192.168.200.1/30'
            data['gre_remote_ip'] = '192.168.200.2'
        else:
            data['gre_local_ip'] = '192.168.200.2/30'
            data['gre_remote_ip'] = '192.168.200.1'
    ...
    def stop(self):
        """Stops one endpoint of a GRE Tunnel
